...
def test_two_buys_open_two_lots_in_order() -> None:
    b1 = make_order_from_fill("f1", "BUY", Decimal("10"), Decimal("180"),
                              trade_time=T0)
    b2 = make_order_from_fill("f2", "BUY", Decimal("5"), Decimal("190"),
                              trade_time=T0 + timedelta(days=1))
...
def test_sell_closes_oldest_lot_first() -> None:
    b1 = make_order_from_fill("f1", "BUY", Decimal("10"), Decimal("180"),
                              trade_time=T0)
    b2 = make_order_from_fill("f2", "BUY", Decimal("10"), Decimal("190"),
                              trade_time=T0 + timedelta(days=1))
    s = make_order_from_fill("f3", "SELL", Decimal("-5"), Decimal("200"),
                             trade_time=T0 + timedelta(days=10))
...
def test_partial_close_leaves_remaining_quantity_in_first_lot() -> None:
    b1 = make_order_from_fill("f1", "BUY", Decimal("10"), Decimal("180"),
                              trade_time=T0)
    s = make_order_from_fill("f2", "SELL", Decimal("-3"), Decimal("200"),
                             trade_time=T0 + timedelta(days=5))
...
def test_sell_spanning_multiple_lots_emits_one_close_per_lot() -> None:
    ...
    b2 = make_order_from_fill("f2", "BUY", Decimal("10"), Decimal("190"),
                              trade_time=T0 + timedelta(days=1))
    s = make_order_from_fill("f3", "SELL", Decimal("-15"), Decimal("200"),
                             trade_time=T0 + timedelta(days=10))
    result = apply_orders([b1, b2, s])
...
def test_commissions_flow_into_cost_basis_and_proceeds() -> None:
    b = make_order_from_fill("f1", "BUY", Decimal("10"), Decimal("100"),
                             commission=Decimal("-2"))
    s = make_order_from_fill("f2", "SELL", Decimal("-10"), Decimal("110"),
...
def test_total_realized_pnl_sums_every_close() -> None:
    b1 = make_order_from_fill("f1", "BUY", Decimal("10"), Decimal("100"),
                              trade_time=T0)
    b2 = make_order_from_fill("f2", "BUY", Decimal("10"), Decimal("110"),
                              trade_time=T0 + timedelta(days=1))
    s = make_order_from_fill("f3", "SELL", Decimal("-15"), Decimal("120"),
                             trade_time=T0 + timedelta(days=10))
    result = apply_orders([b1, b2, s])
...
def test_opening_lots_are_consumed_before_current_year_buys() -> None:
    ...
    new_buy = make_order_from_fill("f1", "BUY", Decimal("10"), Decimal("180"),
                                   trade_time=T0)
    s = make_order_from_fill("f2", "SELL", Decimal("-10"), Decimal("200"),
                             trade_time=T0 + timedelta(days=5))
...
def test_selling_more_than_held_raises_fifo_error() -> None:
    b = make_order_from_fill("f1", "BUY", Decimal("5"), Decimal("100"),
                             trade_time=T0)
    s = make_order_from_fill("f2", "SELL", Decimal("-10"), Decimal("110"),
                             trade_time=T0 + timedelta(days=1))
...
def test_unknown_side_raises() -> None:
    ...
    fill = Fill(
        fill_id="f1", symbol="AAPL", side="CANCEL", quantity=Decimal("1"),
        price=Decimal("100"), money=Decimal("100"), commission=Decimal("0"),
...
def test_apply_orders_is_order_independent() -> None:
    ...
    b2 = make_order_from_fill("f2", "BUY", Decimal("10"), Decimal("110"),
                              trade_time=T0 + timedelta(days=1))
    s = make_order_from_fill("f3", "SELL", Decimal("-5"), Decimal("150"),
                             trade_time=T0 + timedelta(days=2))
    ...
    assert [c.realized_pnl for c in forward.closes] == \
           [c.realized_pnl for c in shuffled.closes]
