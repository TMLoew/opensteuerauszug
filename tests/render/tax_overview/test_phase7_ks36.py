...
def _criteria_sample() -> list[KS36Criterion]:
    return [
        ...
        KS36Criterion(
            ...
            observed_value=D("0.45"), threshold=D("0.50"),
            unit="ratio", triggered=False, status="amber",
        ),
        KS36Criterion(
            code="leverage",
        ...
        KS36Criterion(
            ...
            observed_value=D("2"), threshold=D("0"),
            unit="count", triggered=True, status="amber",
...
def test_ampel_styles_never_appear_on_visible_sheets() -> None:
    ...
    wb = render_workbook(_preparer_data())
    ampel_styles = {
        StyleName.KS36_GREEN, StyleName.KS36_AMBER, StyleName.KS36_RED,
    }
...
def test_preparer_mode_with_no_ks36_data_still_creates_hidden_sheets() -> None:
    data = TaxOverviewData(
        tax_year=2025, broker="ibkr", preparer_mode=True,
