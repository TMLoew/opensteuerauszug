...
def draw_page_header(canvas, doc, is_barcode_page: bool = False):
    ...
    if hasattr(doc, 'tax_statement') and doc.tax_statement:
        ...
        institution_name = (
            doc.company_name if hasattr(doc, 'company_name') and doc.company_name else ""
        )
...
def render_tax_statement(
    ...
    company_name = (
        ...
        if institution_name_override
        else (tax_statement.institution.name if tax_statement.institution else "")
    )
    ...
    if securities_table_a:
        ...
        story.append(
            Paragraph(
                "* Manuell erfasster Preis (nicht aus der offiziellen Kursliste)",
                styles['Val_LEFT'],
            )
        )
        story.append(Spacer(1, 0.5 * cm))
