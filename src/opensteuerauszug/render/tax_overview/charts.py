...
# Design-locked palette first, then deterministic HSL ramp for overflow.
_SLICE_COLORS = [
    PALETTE["primary"],
    PALETTE["accent"],
    PALETTE["primary_80"],
...
def render_pie(allocations: Sequence[SectorAllocation], *, size: int = 280) -> str:
    ...
    parts.append(
        ...
        f'aria-label="Pie chart of sector allocation">'
    )
...
def render_bar(
    ...
    width: int = 720, bar_height: int = 22, top_n: int = 12,
) -> str:
...
def render_benchmark_comparison(
    ...
    *,
    width: int = 720, bar_height: int = 28,
) -> str:
