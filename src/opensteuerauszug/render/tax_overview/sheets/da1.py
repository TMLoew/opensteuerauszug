...
source. The Kanton SG tax office accepts this helper table as the working
document that backs the DA-1 claim — one row per (security, source-country)
bucket with the treaty ceiling and recoverable amount spelled out.
...
def write_da1_sheet(workbook: Workbook, data: TaxOverviewData) -> None:
    ...
    write_header_row(ws, 1, [
        "ISIN", "Symbol", "Bezeichnung", "Land",
        "Bruttoertrag (CHF)", "Quellensteuer (CHF)",
        "Einbehaltener Satz", "Abkommensobergrenze", "Rückforderbar (CHF)",
    ])
    freeze_header(ws, 1)
