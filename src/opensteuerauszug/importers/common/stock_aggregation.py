            ...
            if (
                ...
                if pending.unitPrice != stock.unitPrice:
                    if pending.unitPrice is not None and stock.unitPrice is not None:
                        pending.unitPrice = (
                            pending.quantity * pending.unitPrice + stock.quantity * stock.unitPrice
                        ) / total_quantity
                    else:
                        pending.unitPrice = (
                            pending.unitPrice if pending.unitPrice is not None else stock.unitPrice
                        )
                if pending.exchangeRate != stock.exchangeRate and stock.exchangeRate is not None:
                    # Quantity-weighted FX average, mirroring the unit-price merge.
                    prev_qty = total_quantity - stock.quantity
                    prev_fx = pending.exchangeRate or stock.exchangeRate
                    pending.exchangeRate = (
                        prev_qty * prev_fx + stock.quantity * stock.exchangeRate
                    ) / total_quantity
